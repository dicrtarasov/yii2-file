...
class FileAttributeBehavior extends Behavior
{
    ...
    /** @var array [attibuteName => \app\lib\store\File[]] */
    private $values = [];
     ...
     * @param bool $refresh
     * @return null|\dicr\file\File|\dicr\file\UploadFile|array
     */
    public function getFileAttributeValue(string $attribute, bool $refresh = false)
    {
        if (! isset($this->values[$attribute]) || $refresh) {
            $this->values[$attribute] = $this->store->getModelPath($this->owner, $attribute)->getList([
                'dirs' => false
            ]);
        }
     ...
     *
     * @param string $attribute
     * @param null|array|\dicr\file\File|\dicr\file\UploadFile $value
     * @return static
     */
     ...
     *
     * @param string $attribute
     *            имя загружаемого файлового аттрибута
    ...
    public function loadFileAttribute(string $attribute, string $formName = null)
    {
        ...
        $post = ArrayHelper::getValue(\Yii::$app->request->post(), [
            $formName,
            $attribute
        ], null);
        ...
        if (! empty($post)) {
            foreach ((array) $post as $pos => $name) {
        ...
        if (! empty($files)) {
            foreach ((array) $files as $pos => $file) {
                $value[$pos] = $file;
            }
        }
    ...
    public function validateFileAttribute(string $attribute)
    {
        ...
        if (empty($files)) {
            ...
            $files = [
                $files
            ]; // нельзя объект преобразовывать через (array)
        }
        ...
        foreach ($files as $file) {
            ...
            if (empty($file)) {
                ...
                /** @var \app\lib\store\UploadFile $file */
                if (! empty($file->error)) {
    ...
    public function saveFileAttribute(string $attribute)
    {
        ...
        /** @var \app\lib\store\File[] $oldFiles старые файлы */
        ...
        foreach ($files as $pos => $file) {
            ...
            if (empty($file)) {
                ...
                if (! in_array($file->name, $oldNames)) {
                    unset($files[$pos]);
                } else {
                    ...
                    $matches = null;
                    $file->setName(sprintf('.%d~%s', rand(100000, 999999), preg_match('~^\d+\~(.+)$~uism', $file->name, $matches) ? $matches[1] : $file->name), true);
        ...
        foreach ($oldFiles as $oldFile) {
            if (! in_array($oldFile->name, $saveNames)) {
                $oldFile->delete();
            }
        }
        ...
        foreach (array_values($files) as $pos => &$file) {
            ...
            if (! preg_match('~^\.\d+\~(.+)$~uism', $file->name, $matches)) {
                throw new ServerErrorHttpException('внутренняя ошибка: ' . $file->name);
            }
            $file->setName(sprintf('%d~%s', $pos, $matches[1]), true);
        }
